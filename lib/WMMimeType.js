...
WMMimeType["getType"] = getType; // WMMimeType.getType(ext:IgnoreCaseString):MimeTypeString

// --- implements ------------------------------------------
function isText(ext) { // @arg IgnoreCaseString - "ext" or ".ext"
                       // @ret Boolean
    return /^text/.test(getType(ext));
}

function isFont(ext) { // @arg IgnoreCaseString - "ext" or ".ext"
                       // @ret Boolean
    return /font/.test(getType(ext));
}

function isImage(ext) { // @arg IgnoreCaseString - "ext" or ".ext"
                        // @ret Boolean
    return /^image/.test(getType(ext));
}

function isAudio(ext) { // @arg IgnoreCaseString - "ext" or ".ext"
                        // @ret Boolean
    return /^audio/.test(getType(ext));
}

function isVideo(ext) { // @arg IgnoreCaseString - "ext" or ".ext"
                        // @ret Boolean
    return /^video/.test(getType(ext));
}

function getType(ext) { // @arg IgnoreCaseStrign - file extension. "ext" or ".ext"
                        // @ret String - mime type
//{@dev
    $valid($type(ext, "String"), getType, "ext");
//}@dev

    ext = ext.replace(/^./, "").toLowerCase();
    return MIMETYPES[ext] || MIMETYPES["bin"];
}
